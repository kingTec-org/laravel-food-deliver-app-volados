	...
	<div class="partners">
		...
		<div class="pickup-times my-md-4 panel-content">
			...
			<div class="mt-4" ng-init="day_name ={{ json_encode(day_name()) }};preparation_timing={{ json_encode($preparation) }};max_time= {{ $max_time }} ? {{ $max_time }}  :  50" >
				<h2>{{ trans('messages.store_dashboard.average_item_preparation_times') }}</h2>
				<p>{{ trans('messages.store_dashboard.let_know_how_long_it_usually_takes') }}</p>
				<form action="{{route('store.update_preparation_time')}}" method="post" id="store_preparation_time">
					@csrf
